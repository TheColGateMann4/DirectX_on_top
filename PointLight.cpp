...
void PointLight::SpawnControlWindow(GFX& gfx, float deltaTime) noexcept
{
	if (ImGui::Begin("Light"))
	{
		...
		if (enableChroma)
		{
			ImGui::SliderFloat("Chroma Delta Time", &chromaDeltaTime, 0.000001f, 80.0f, "%.6f");
		}
		...
		if (enableChroma && deltaTime - lastDeltaTime >= chromaDeltaTime)
		{
			...
			if(!justSwitched)
				if (incrementingNow)
					*pIncrementColor += 0.01f;
				else
					*pDecrementColor -= 0.01f;
