...
float3 GetPBR(const float3 N, const float3 V, const float3 L, const float3 H, const float3 reflectivity, const float3 emission, const float3 diffuseColor, const float metalic, const float roughness, float2 textureCoords : TEXCOORD, float4 depthMapCoords : DEPTHTEXCOORD)
{
