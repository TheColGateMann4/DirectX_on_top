...
BOOL Window::MultiselectToFilePaths(std::string* multiSelectStr, std::vector<std::string>* filePaths)
{
	...
	if (currentEndPos + 1 < multiSelectStr->length()) // only one file was selected
		if (multiSelectStr->at(currentEndPos + 1) == '\0')
		{
 			if (filepath.length() <= 4 || std::string(filepath.end() - 4, filepath.end()) != ".obj") // if its just weirdly short or doesn't have .obj extension, we don't return anything
 				return 0;
	...
	for (size_t i = 0; i < multiSelectStr->length();i++)
	{
		...
		if(fileNameAndExtension.length() > 4)
			if(std::string(fileNameAndExtension.end() - 4, fileNameAndExtension.end()) == ".obj")
