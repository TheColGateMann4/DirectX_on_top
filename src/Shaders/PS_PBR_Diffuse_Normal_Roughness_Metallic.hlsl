...
cbuffer ObjectBuffer : register(b1)
{
    ...
    float3 b_subsurfaceColor;     // Color for subsurface scattering
    float b_subsurfaceIntensity;  // Intensity of subsurface scattering
    float b_scatteringRadius;     // Scattering radius (affects how far light penetrates)

    float b_rimPower;
    float b_rimIntensity;
};
...
//        float distanceFactor = exp(-b_scatteringRadius * NdotL);
//        float3 diffuseLight = lambert * NdotL * lightColor;
//        float3 scatteredLight = b_subsurfaceColor * distanceFactor * b_subsurfaceIntensity;
//        outgoingLight += scatteredLight;
//    }

//    // rim 
//    {
//        float NdotL = max(dot(N, L), 0.0f);
//        float VdotL = max(dot(N, V), 0.0f);
//
//        float rim = pow(1.0 - saturate(NdotL), b_rimPower);
//        //rim = max(rim * (1.0F - VdotL), 0.0f);
//
//        outgoingLight += lightColor * rim * b_rimIntensity;
//    }
